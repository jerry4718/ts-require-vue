...
let file_path = path.join(__dirname, "../../src/app/common/api/api.ts");

let file_context = fs.readFileSync(file_path).toString();

for (let {name, code} of codes) {
    while (file_context.indexOf(code) > -1) {
        file_context = file_context.replace(code, name);
    }
}
